import React, {useContext, useCallback, useMemo, useState, useLayoutEffect} from 'react';
import {Page, BlockTitle, List, ListItem, Navbar, NavLeft, NavRight, Panel, View, Link, NavTitle, Fab, Icon, Button, Actions, ActionsGroup, ActionsButton, Sheet, Block} from 'framework7-react';
...
const actionsDef = {
    ...
    [ACTIONS.IMPERSONATE]: {title: 'Impersonate', action: (m, ctx) => ctx.changeLogin(m.login), icon: 'enter_round', canApply: (m, user) => (m.bot && (user.host || user.bot)) || (m.host && user.bot)},
    [ACTIONS.ASSASSIN]: {title: 'Assassinate', action: (m, ctx) => gameApi.assassinAction(ctx.roomId, m.login), icon: 'bolt_fill'},
    [ACTIONS.DEEP_AGENT]: {title: 'Blame', action: (m, ctx) => gameApi.deepAgentAction(ctx.roomId, m.login), icon: 'hand_alt'},
    [ACTIONS.HUNTER]: {title: 'Accuse', action: (m, ctx) => gameApi.hunterAction(ctx.roomId, m.login), icon: 'bolt_fill'},
    [ACTIONS.SPY_HUNTER]: {title: 'Accuse', action: (m, ctx) => gameApi.spyHunterAction(ctx.roomId, m.login), icon: 'bolt_fill'},
...
export default (props) => {
    ...
    return (
        <Page name="members">
            ...
            {members && <List mediaList sortable className={`${showSort? 'sortable-enabled' : ''}`} onSortableSort={onReorder}>
                {members.map((m,i) => {
                    ...
                    return (<ListItem key={`${m.login} ${actions[i] && actions[i].length} ${vision[i]}`} badgeColor="var(--f7-theme-color)" {...props}>
                        {roleIcons[vision[i]]({slot: 'media', size: 40})}
                        {!showSort && actions[i] && actions[i].length===1 && <Button slot="after" onClick={actions[i][0].action}><Icon f7={actions[i][0].icon}/></Button>}
                    </ListItem>)

                })}
            </List>}

            {status === STATUS.NEW && user && user.host && <Fab position="right-bottom" slot="fixed" text="Add bot" color="green" onClick={addBot}>
                <Icon ios="f7:add" aurora="f7:add" md="material:add"></Icon>
            </Fab>}

            <Actions opened={openedId!==null} onActionsClosed={() => setOpenedId(null)}>
                <ActionsGroup>
                    {openedId!==null && actions[openedId] && actions[openedId].map(a => <ActionsButton bold onClick={a.action} key={a.title}>{a.title}</ActionsButton>)}
                    <ActionsButton color="red">Cancel</ActionsButton>
                </ActionsGroup>
            </Actions>

            <Sheet swipeToClose backdrop opened={investResult!==null} onSheetClosed={onInvestClose} className="middle-aligned-sheet">
                {investResult && <Block string className={`invest-result ${investResult.loyalty===LOAYLTY.RESISTANCE_CHIEF ? 'color-resistance' : ''} ${investResult.loyalty===LOAYLTY.SPY_CHIEF ? 'color-spies' : ''}`}>
