...
import {AccordionContent, Block, Chip, Icon, List, ListItem, Navbar, NavTitle, Page} from "framework7-react";
                        ...
                        <List>
                            {members.map((m, i) => (
                                <ListItem key={i} title={m.login}>
                                    ...
                                    {mission.participants.includes(i) &&
                                    <Icon slot="after" f7="bolt_fill" color="blue"/>}
                                    {mission.status !== OP_STATUS.VOTE && mission.vote && mission.vote[i] === VOTE.YES &&
                                    <Icon slot="after" f7="thumbs_up" color="green"/>}
                                    {mission.status !== OP_STATUS.VOTE && mission.vote && mission.vote[i] === VOTE.NO &&
                                    <Icon slot="after" f7="thumbs_down" color="red"/>}
                                </ListItem>
                            ))}
                        </List>
                        {mission.summary && <Block className="mission-result-chips">
                            {Object.entries(mission.summary).map(([k, v]) => v > 0 &&
                                <Chip text={options[k].title} media={'' + v} key={k} color={options[k].color}/>)}
                        </Block>}
                    </AccordionContent>
                </ListItem>)}
            </List>}
