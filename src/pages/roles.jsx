...
import {Page, BlockTitle, Navbar, NavTitle, Block, Row, Col} from 'framework7-react';
import {AppContext} from "@/components/app";
import {EXPANSION, STATUS, TEAM} from "@/api/enum";
    ...
    return (
        <Page>
            ...
            {isProperRoomSize(members.length) && <>
                ...
                <Block strong>
                    <Row>
                        <Col>{composition[TEAM.BAD].map((r,i) => <p key={`${i} ${r}`} className="color-spies text-align-center">{roleTraits[r].title}</p>)}</Col>
                        <Col>{composition[TEAM.GOOD].map((r,i) => <p key={`${i} ${r}`} className="color-resistance text-align-center">{roleTraits[r].title}</p>)}</Col>
                    </Row>
