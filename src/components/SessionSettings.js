...
import {BlockTitle, List, ListItem, Button, Block} from "framework7-react";
...
export function SessionSettings(props) {
    ...
    return (
        <>
            ...
            <List>
                <ListItem radio title="Classic" name="module" value={EXPANSION.CLASSIC} onChange={onModuleSelect} checked={module===EXPANSION.CLASSIC}/>
                <ListItem radio title="Assassin" name="module" value={EXPANSION.ASSASSIN} onChange={onModuleSelect} checked={module===EXPANSION.ASSASSIN}/>
                <ListItem radio title="Hunter" name="module" value={EXPANSION.HUNTER} onChange={onModuleSelect} checked={module===EXPANSION.HUNTER}/>
            </List>
            {room.members && room.members.length >= 5 && <>
                <BlockTitle>Optional roles</BlockTitle>
                <List>
                    {rolesDescription.map(r =>
                        <ListItem checkbox key={r.id} disabled={!baseCount[r.side] && !roles.includes(r.id)}  checked={roles.includes(r.id)} onChange={onRoleToggle} name="role" value={r.id}>
