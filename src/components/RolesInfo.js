...
import {BlockTitle, List, ListItem, AccordionContent, Block} from "framework7-react";
...
export function RolesInfo(props) {
    ...
    return (
        <>
            ...
            <List accordionList>
                {roleDescriptions.map(r => <ListItem color={teamTraits[r.side].color} key={r.id} accordionItem title={r.title} cl>
                        <AccordionContent>
                            <Block>
                                <p>{r.description}</p>
                            </Block>
