...
import {OP_STATUS, STATUS, TEAM} from '@/api/enum';
        ...
        <AppContext.Provider value={this.state.roomData}>
            ...
            <LoginScreen opened={!this.state.cred}>
                <Page loginScreen>
                    ...
                    <List form>
                        <ListInput ref="login" label="Your name" errorMessageForce={Boolean(errors.login)} type="text" placeholder="John Smith" value={login} onInput={e=>this.setState({login:e.target.value})}>
                            {errors.login && errors.login.map(e => <span slot="error-message" key={e}>{e}</span>)}
                        </ListInput>
                        <ListItem radio value={TEAM.GOOD} name="side" title="Resistance" onChange={e=>this.setState({side: e.target.value})}/>
                        <ListItem radio value={TEAM.BAD} name="side" title="Spies" onChange={e=>this.setState({side: e.target.value})}/>
                        <ListItem radio value={TEAM.RANDOM} name="side" title="Random side" defaultChecked onChange={e=>this.setState({side: e.target.value})}/>
                    </List>
                    {roomId && <List>
      ...
      this.removeListener = gameApi.listenRoom(cred.roomId, snapshot => {
        ...
        this.setState({
            roomData: {
                ...
                changeLogin: (login) => {
                    this.setState({login, roomData: {...this.state.roomData, user: room.members.find(m => m.login === login)}})
                }
            }
        })
      })
